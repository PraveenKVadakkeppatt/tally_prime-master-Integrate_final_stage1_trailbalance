...
def trialbalance(request):
    if 't_id' in request.session:
        ...
        total=vouchert1.objects.filter(company_id=t_id)
        ...
        totinc=inincvouchert.objects.filter(company_id=t_id)
        ...
        totinx=ininxvouchert.objects.filter(company_id=t_id)
...
def incgroupsummary(request):
    if 't_id' in request.session:
        if request.session.has_key('t_id'):
            ...
            return redirect('/')
        tally = Companies.objects.filter(id=t_id) 
        vouch=inincvouchert.objects.filter(company_id=t_id)
...
def inincvoucher(request):
    if 't_id' in request.session:
        ...
        led1=tally_ledger.objects.filter(company_id=t_id)
        context={'led':led,'led1':led1}   
    return render(request,'inincvoucher.html',context)    
...
def incledgersummary(request,pk):
    if 't_id' in request.session:
        ...
        vch=inincvouchert.objects.get(company_id=t_id)
...
def ininxvoucher(request):
    if 't_id' in request.session:
        ...
        led1=tally_ledger.objects.filter(company_id=t_id)
        context={'led':led,'led1':led1}
    return render(request,'ininxvoucher.html',context)  
...
def inxgroupsummary(request):
    if 't_id' in request.session:
        if request.session.has_key('t_id'):
            ...
            return redirect('/')
        tally = Companies.objects.filter(id=t_id) 
        vouch=ininxvouchert.objects.filter(company_id=t_id)
        sum1=0
        sum2=0
        for a in vouch:
            sum1+=a.icredit
...
def groupsummery1(request):
    if 't_id' in request.session:
        ...
        vouch=vouchert1.objects.filter(company_id=t_id)
        ...
        context={'vouch':vouch,'sum1':sum1,'sum2':sum2}    
    return render(request,'groupsummery1.html',context)

def ledgersummary1(request,pk):
    vch=vouchert1.objects.get(id=pk)  
    vouch=vouchert1.objects.filter(id=pk)
    ...
    context={'vch':vch,'vouch':vouch,'sum1':sum1,'sum2':sum2,'sum3':sum3,'sum3':sum3,'tot':tot,'cltot':cltot}      
    return render(request,'ledgersummary1.html',context)

def voucheradd1(request):
    if 't_id' in request.session:
        if request.session.has_key('t_id'):
            t_id = request.session['t_id']
        else:
        ...
        led1=tally_ledger.objects.filter(company_id=t_id)
        context={'led':led,'led1':led1}   
    return render(request,'voucheradd1.html',context)    

def vouchadd1(request):
    if 't_id' in request.session:
        if request.session.has_key('t_id'):
            t_id = request.session['t_id']
        else:
        ...
        if request.method == 'POST':
            ...
            vou.save()
        return redirect('voucheradd1')

def ledgervoucher1(request,pk):
    vch=vouchert.objects.get(id=pk)
    vouch=vouchert.objects.filter(id=pk)
